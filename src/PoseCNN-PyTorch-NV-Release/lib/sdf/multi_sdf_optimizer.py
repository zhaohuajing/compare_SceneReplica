...
class multi_sdf_optimizer():
    def __init__(self, sdf_file, lr=0.01, online_calib=True, use_gpu=False):
        ...
        if use_gpu:
            self.dpose = torch.tensor([0, 0, 0, 1e-12, 1e-12, 1e-12], dtype=torch.float16, requires_grad=True, device=0)
        else:
            self.dpose = torch.tensor([0, 0, 0, 1e-12, 1e-12, 1e-12], dtype=torch.float16, requires_grad=True)

        self.online_calib = online_calib
        if online_calib:
            if use_gpu:
                self.dpose_ext = torch.tensor([0, 0, 0, 1e-12, 1e-12, 1e-12], dtype=torch.float16, requires_grad=True,
                                              device=0)
            else:
                self.dpose_ext = torch.tensor([0, 0, 0, 1e-12, 1e-12, 1e-12], dtype=torch.float16, requires_grad=True)
        else:
            self.dpose_ext = torch.tensor([0, 0, 0, 1e-12, 1e-12, 1e-12], dtype=torch.float16, requires_grad=False)
            if use_gpu:
