...
# NOTE from Zhao 25/07/31: Modified all dtype=torch.float32 to dtype=torch.float16 in an attempt to debug cuda outof memory issue, then reverted due to lib/layers/sdf_matching_loss.py "outputs = posecnn_cuda.sdf_loss_forward(pose_delta, pose_init, sdf_grids, sdf_limits, points, regularization) RuntimeError: expected scalar type Float but found Half
