...
def get_backbone(pretrained_file=pretrained_path):
    ...
    if pretrained:
        # network_data = torch.load(pretrained) # modified 25/08/01
        network_data = torch.load(pretrained, weights_only=True)
...
def get_backbone_crop(pretrained_crop_file=pretrained_crop_path):
    ...
    if pretrained_crop:
        # network_data_crop = torch.load(pretrained_crop) # modified 25/08/01
        network_data_crop = torch.load(pretrained_crop, weights_only=True)
