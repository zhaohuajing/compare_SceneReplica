...
def crop_rois(rgb, initial_masks, depth):
    ...
    for index, mask_id in enumerate(mask_ids):
        ...
        # rgb_crop = F.upsample_bilinear(rgb_crop.unsqueeze(0), new_size, mode="bilinear", align_corners=True)[0] # Modified 25/07/25 by Zhao
...
def match_label_crop(initial_masks, labels_crop, out_label_crop, rois, depth_crop):
    ...
    for index in sorted_ids:
        ...
        # resized_mask = F.upsample_nearest(mask, (orig_H, orig_W), mode="nearest")[0, 0]

        # Set refined mask
        h_idx, w_idx = torch.nonzero(resized_mask).t()
        
        # Modified 25/07/25 by zhao
        ...
        resized_mask = resized_mask.to(refined_masks.device)  # Ensure mask is on same device
        h_idx = h_idx.to(refined_masks.device)                # Ensure indices are also on same device
        w_idx = w_idx.to(refined_masks.device)
        refined_masks[0, y_min:y_max+1, x_min:x_max+1][h_idx, w_idx] = resized_mask[h_idx, w_idx]
