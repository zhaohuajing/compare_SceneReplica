...
def connected_components(Z, epsilon, metric='cosine'):
    ...
    for i in range(n):
        if cluster_labels[i] == -1:
            ...
            # if torch.unique(cluster_labels[component_seeds]).shape[0] > 1:    # Edited 25/07.25 by Zhao:
            if torch.unique(cluster_labels[component_seeds.to(cluster_labels.device)]).shape[0] > 1:
                # temp = cluster_labels[component_seeds].numpy()   # Edited 25/07.25 by Zhao:
                temp = cluster_labels[component_seeds.to(cluster_labels.device)].cpu().numpy()
...
def mean_shift_smart_init(X, kappa, num_seeds=100, max_iters=10, metric='cosine'):
    ...
    # cluster_labels = seed_cluster_labels[closest_seed_indices] # Edited 25/07/25 by Zhao
    cluster_labels = seed_cluster_labels[closest_seed_indices.to(seed_cluster_labels.device)]
